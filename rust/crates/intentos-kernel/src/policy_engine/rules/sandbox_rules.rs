...
fn evaluate_sandbox_mode(token: &VerifiedToken, _syscall: &IkSyscall) -> PolicyResult {
    if is_sandboxed(token) {
        ...
        PolicyResult::Allow {
            evidence: vec![Evidence::SandboxUnisolated],
        }
    }
}
...
mod tests {
    ...
    fn sandbox_mode_emits_isolated_for_sandboxed_principal() {
        ...
        assert!(
            matches!(result, PolicyResult::Allow { evidence } if evidence.contains(&Evidence::SandboxIsolated))
        );
    }
    ...
    fn sandbox_mode_emits_unisolated_for_normal_principal() {
        ...
        assert!(
            matches!(result, PolicyResult::Allow { evidence } if evidence.contains(&Evidence::SandboxUnisolated))
        );
    }
