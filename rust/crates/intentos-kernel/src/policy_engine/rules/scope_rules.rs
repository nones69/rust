...
fn evaluate_scope(token: &VerifiedToken, syscall: &IkSyscall) -> PolicyResult {
    if token.scope.permits(syscall) {
        PolicyResult::Allow {
            evidence: vec![Evidence::ScopeMatch { scope: scope_label(&token.scope) }],
        }
    } else {
...
mod tests {
    ...
    fn fs_scope_allows_matching_read() {
        ...
        let syscall = IkSyscall::IkOpen { path: "/tmp/a.txt".into(), mode: OpenMode::Read };
    ...
    fn fs_scope_denies_out_of_prefix() {
        ...
        let syscall = IkSyscall::IkOpen { path: "/etc/passwd".into(), mode: OpenMode::Read };
        let rule = fs_scope_rule();
