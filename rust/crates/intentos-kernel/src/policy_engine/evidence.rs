...
mod tests {
    ...
    fn labels_are_non_empty() {
        let samples = [
            Evidence::ScopeMatch { scope: "fs:/tmp".into() },
            Evidence::ScopeMismatch { scope: "net:example.com".into() },
            Evidence::QuotaRemaining { bytes: 1024, requests: 10 },
            Evidence::QuotaExceeded { field: "bytes".into() },
            Evidence::TTLValid,
            Evidence::TTLExpired,
            Evidence::TokenValid,
            Evidence::TokenExpired,
            Evidence::RuleApplied { rule_id: "ttl-check".into() },
            Evidence::SandboxIsolated,
            Evidence::SandboxUnisolated,
            Evidence::FederationTrusted { peer: "node-a".into() },
            Evidence::FederationUntrusted { peer: "node-b".into() },
