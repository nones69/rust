    ...
    pub fn summary(&self) -> String {
        if self.allow {
            format!(
                ...
                self.evidence.iter().map(|e| e.label()).collect::<Vec<_>>().join(",")
            )
        } else {
            format!(
...
/// `true`.
///
/// If the registry is empty the call is **allowed** (fail-open for
/// no-policy environments); add an explicit deny-all rule when you need
...
pub fn evaluate(
    ...
    for rule in &registry.rules {
        ...
        match result {
            ...
            PolicyResult::Deny { reason, evidence: ev } => {
                evidence.extend(ev);
                evidence.push(Evidence::RuleApplied { rule_id: rule.id.clone() });
                return IkpeDecision {
                    allow: false,
                    evidence,
                    rule_chain: chain,
...
mod tests {
    ...
    fn open_read() -> IkSyscall {
        IkSyscall::IkOpen { path: "/tmp/x.txt".into(), mode: OpenMode::Read }
    }
    ...
    fn single_deny_rule_short_circuits() {
        ...
        assert!(d.evidence.iter().any(|e| matches!(e, Evidence::RuleApplied { rule_id } if rule_id == "deny-all")));
    }
    ...
    fn multiple_allow_rules_accumulate_evidence() {
        ...
        reg.register(PolicyRule {
            ...
            evaluate: |_, _| PolicyResult::Allow {
                evidence: vec![Evidence::ScopeMatch { scope: "fs:/tmp".into() }],
            },
        });
        reg.register(PolicyRule {
            id: "ttl-check".into(),
        ...
        assert!(d.evidence.iter().any(|e| matches!(e, Evidence::ScopeMatch { .. })));
        assert!(d.evidence.contains(&Evidence::TTLValid));
    }
    ...
    fn summary_is_non_empty_for_allow_and_deny() {
        let reg = RuleRegistry::new();
        let allow_d = evaluate(&reg, &test_token(), &open_read());
        assert!(!allow_d.summary().is_empty());
