...
use intentos_kernel::{
    Handle, Intent, KernelError, SyscallRequest, SyscallResult, TrustAnchor,
    SyscallOp as KernelSyscallOp, Kernel,
};
...
impl ServerState {
    ...
    fn admit(&mut self, principal: &str) -> bool {
        let entry = self.quotas.entry(principal.to_string()).or_insert(QuotaEntry {
    ...
    fn increment(&mut self, principal: &str) {
        self.quotas
            ...
            .or_insert(QuotaEntry { active_connections: 0, total_requests: 0 })
            .total_requests += 1;
    }
}
...
async fn handle_connection(
    ...
    if quota_exceeded {
        ...
        let _ = ch.send_json(&KernelRpcResponse::denied("quota exceeded")).await;
        if let Some(audit) = kernel.audit_ref() {
            let _ = audit.record(
                AuditEventKind::RemoteRejected,
                &peer.principal,
...
async fn dispatch(
    ...
    match req {
        ...
        RenewLease { lease_id, ttl_ms, .. } => match kernel.renew_lease(&lease_id, ttl_ms) {
