...
fn load_certs(path: &Path) -> Result<Vec<CertificateDer<'static>>> {
    let file =
        std::fs::File::open(path).with_context(|| format!("open cert file {}", path.display()))?;
...
impl SecureChannel {
    /// SHA3-256 fingerprint of the peer leaf certificate, if presented.
    ...
    pub async fn connect(addr: &str, config: &TlsConfig) -> Result<Self> {
        ...
        bail!("{}", TransportError::UnsupportedAddress(addr.to_string()))
    }
